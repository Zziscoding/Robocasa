    ...
    *,
    ...
    if not used_process_pool:
        if workers <= 1:
            thread_results = []
            for job in jobs:
                thread_results.append(_thread_run(job))
                if progress_callback is not None:
                    try:
                        progress_callback(1)
                    except Exception:
...
class BaseAlignmentResult:
    """Outcome of aligning the mobile base to the demonstration EE pose.

    Ported from ``mink_solver.BaseAlignmentResult`` so the open-drawer chain can
    consume a single entry surface (``mink_q``).
    """
...
class PreContactMinkSolution:
    """Result of solving a pre-contact arm q near a target EE pose.

    Ported from ``mink_solver.PreContactMinkSolution``. It is intentionally a
    plain (non-frozen) dataclass so downstream code may still attach extra
    attributes such as ``solution.gripper_opening = ...``.
    """
...
def _align_base_joint_addresses(model):
    names = (
...
def solve_precontact_pose(
    ...
    """Solve a pre-contact arm q near a target EE pose.

    Equivalent to ``mink_solver.solve_precontact_pose`` but routed through
    ``mink_q``'s own 7-DOF frame-pose solver so the open-drawer pipeline uses
    the same entry surface as the close-drawer pipeline (``mink_q``).
    """
    close_demo = _close_demo()
    frame_name = str(
    ...
    q_start = _write_arm_q(seed_arm_q)
    q_posture = _write_arm_q(np.asarray(robot_state["q"], dtype=np.float64))
...
def align_base_to_demo_ee_pose(
    ...
    """Align the mobile base to the demonstration EE pose.

    Ported from ``mink_solver.align_base_to_demo_ee_pose``; this routine does
    not call ``mink`` at all, it only nudges the three mobile-base joints.
    """
    close_demo = _close_demo()
    ...
    addresses = _align_base_joint_addresses(env.sim.model)
...
__all__ = [
    ...
    "BaseAlignmentResult",
    "PreContactMinkSolution",
    "solve_precontact_pose",
    "align_base_to_demo_ee_pose",
]
