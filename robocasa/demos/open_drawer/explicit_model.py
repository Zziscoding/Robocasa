...
class ExplicitModel:
    ...
    def init_utils(self):
        ...
        next_robot_qpos = (
            qpos[-(self.param_.n_robot_qpos_) :]
            + self.param_.h_ * qvel[-(self.param_.n_robot_qpos_) :]
        )
        next_obj_quat = (
            qpos[3:7]
            + 0.5 * self.param_.h_ * self.cs_qmat_body_fn_(qpos[3:7]) @ qvel[3:6]
        )
    ...
    def init_model(self):
        ...
        # contact_force = -model_params @ (jac_mat @ Q_inv @ b + phi_vec) - 0.01 * model_params @ jac_mat @ Q_inv @ b / h
        # beta = 100.0
        ...
        self.step_once_fn = cs.Function(
            "step_once", [curr_q, cmd, phi_vec, jac_mat, model_params], [next_qpos]
        )
