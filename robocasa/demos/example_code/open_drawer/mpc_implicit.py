...
from robocasa.demos.open_drawer.qp_model import QPModel
...
class MPCImplicit:
    ...
    def plan_once(self, target_p, target_q, curr_x, phi_vec, jac_mat, sol_guess=None):
        if sol_guess is None:
            sol_guess = dict(
                x0=self.nlp_w0_, lam_x0=self.nlp_lam_x0_, lam_g0=self.nlp_lam_g0_
            )
    ...
    def init_MPC(self):
        ...
        jac_mat = cs.SX.sym(
            "contact_jacobians", self.param_.max_ncon_ * 4, self.param_.n_qvel_
        )
        ...
        dyn_equ_fn = cs.Function(
            "dyn_equ_fn", [v, u, lam, phi_vec, jac_mat], [cs.gradient(dyn_lag, v)]
        )
        dyn_comple_fn = cs.Function(
            "dyn_comple_fn",
            [v, u, lam, phi_vec, jac_mat],
        ...
        for k in range(self.param_.mpc_horizon_):
            ...
            g += [
                dyn_comple_fn(vk, uk, lamk, phi_vec, jac_mat) - self.param_.comple_relax
            ]
        ...
        self.nlp_params_fn_ = cs.Function(
            "nlp_params_fn", [q0, phi_vec, jac_mat, cost_params], [nlp_params]
        )
